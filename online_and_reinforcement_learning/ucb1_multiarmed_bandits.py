...
for DELTA in [1/4, 1/8, 1/16]:
    ...
    def genRewards():
        ...
        current_rewards[0] = np.random.binomial(size=1, n=1, p=0.5 + (0.5 * DELTA))  # gen a*
        current_rewards[1] = np.random.binomial(size=1, n=1, p=0.5 - (0.5 * DELTA))  # gen a
    ...
    for run in range(20):
        ...
        for t in range(T):
            ...
            if t == 0:
                ...
                opt_regret[run].append(opt_regret[run][t - 1] + opt_delta_values[opt_At])
